...
int main()
{
    ...
    // read input file
    ifstream InputFile("onephase feasible input.txt"); //Change to file name as necessary
    string input;
    ...
    if (unbounded)
    {
        // Further output will not be displayed because the matrix cannot be optimized
        return 0;
    }
    ...
    for (int i = 0; i < optimizedMatrix.size(); i++)
    {
        ...
        {
            printf("%0.2f\t", optimizedMatrix[i][j]);
        }
        cout << endl;
    }
...
vector<vector<double>> rowOperations(vector<vector<double>> m)
{
    ...
    else
    {
        ...
        for (int i = 1; i < m.size(); i++)
        {
            // start i at 1 because objective function should not be checked
            if (m[i][index] > 0)
            {
                unbounded = false;
                break;
